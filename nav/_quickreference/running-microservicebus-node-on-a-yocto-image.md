...
## 2. SSH-Keys
The Raspberry Pi setup use SSH-keys for authorization when connection with SSH from host. If you already have an public and private key pair you can skip this section.
...
3. Specify an pass phrase to protect the private key, this is optional
...
2. After completion the boot partition should appear under Computer in the explorer, may use raspberrypi as label.
...
4. Save file to the boot partition or USB flash drive as "authorized_keys"(no file ending)
...
2. Edit country code to match you [location](https://en.wikipedia.org/wiki/ISO_3166-1)
...
5. Save the to the boot partition or USB flash drive named "wpa_supplicant-wlan0.conf"
...
3. If you use [Notpad++](https://notepad-plus-plus.org/download/v7.7.html) change end of line (EOL) to Unix, go to Edit->EOL Conversion->Unix (LF)
4. Save the to the boot partition or USB flash drive named "microservicebus-node.service"
...
2. Now you need to get the IP address of your Raspberry Pi. If you have access to the network router it is convenient to look att DHCP lease. An alternative is to scan your network using an IP-scanner like nmap or [Angry IP Scanner](https://angryip.org/download/#windows). If you have an display and keyboard connected you can simply log in using root and no password, then write ipconfig on the command line and press enter.
``` shell
...
6. Press open to start SSH session, if prompt for user name enter root
