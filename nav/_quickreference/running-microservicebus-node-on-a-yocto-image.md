...
In the rootfs partitions all Linux user space software is installed. There is two rootfs partitions to be able to update and swap firmware, only one is active at a time. The software most interesting for us is *microServiceBus-node* and *RAUC*. *MicroServiceBus-node* is responsible to install and update *microServiceBus-core*. *MicroServiceBus-core* will check for new firmware, if newer firmware exists *microServiceBus-core* will download firmware and then call *RAUC* to install and swap rootfs next boot.
...
Yocto build system runs on Linux but this tutorial will use pre build files and is targeted Windows as an host system. In this tutorial we will use target to reference the Raspberry Pi and host to reference the PC used to write SD-card.
...
1. After completion the boot partition should appear under Computer in the explorer, may use raspberrypi as label.
...
Your file should look something like below except as one line:
``` shell
...
1. Edit country code to match you [location](https://en.wikipedia.org/wiki/ISO_3166-1)
1. Edit ssid and psk to match your WiFi SSID and password
1. If you use [Notpad++](https://notepad-plus-plus.org/download/v7.7.html) change end of line (EOL) to Unix, go to Edit->EOL Conversion->Unix (LF)
1. Save the to the boot partition or USB flash drive named "wpa_supplicant-wlan0.conf"
...
1. If you use [Notpad++](https://notepad-plus-plus.org/download/v7.7.html) change end of line (EOL) to Unix, go to Edit->EOL Conversion->Unix (LF)
1. Save the to the boot partition or USB flash drive named "microservicebus-node.service"
...
1. Go to [mSB.com/Nodes](https://microservicebus.com/Nodes) and check if your device is online. If online congratulations, if not go to next section to be able to debug your node
...
![Firmware upload](/images/running-microservicebus-node-on-a-yocto-image/msb-com-firmware-images.png)
1. To be able to see the update process on the node you can run the command below in the shell:
    ...
    ```
1. Trigger an firmware update by pressing Action->More options->Update firmware on the node in [mSB.com/Nodes](https://microservicebus.com/Nodes). Now will the following process run on your node:
    ...
    1. When mSB-core is installed the node will fetch all flows from mSB.com
    1. Done!
  ...
  1. When the firmware update is done go to [mSB.com/Nodes](https://microservicebus.com/Nodes) open Actions->Properties->Environment and unfold "raucState" here you can see the properties for all RAUC slots(rootfs0/1)
      ...
      ```
      In rootfs0 you should see "state": "inactive" and in rootfs1 "state": "booted, this indicates that your node now have booted to your new firmware
  ...
  ![Firmware upload](/images/running-microservicebus-node-on-a-yocto-image/msb-com-firmware-update-force.png)
